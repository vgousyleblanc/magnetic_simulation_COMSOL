...
model.parameter('V_z1', '-5.02[V]')
model.parameter('V_z2', '-8.29[V]')

model.parameter('V_x12', '-1.4[V]')
model.parameter('V_x22', '-1.382[V]')

model.parameter('V_y1', '-1.172[V]')
model.parameter('V_y2', '-1.20[V]')
...
if scan=='v' and dim=='z':
    print('Starting the voltage scan in the '+str(dim)+'  dimension')
    model.solve('Z_sweep')
    file_name='z_table.txt'
    model.export('Z_table')
    v0,v1,mx,my,mz=np.loadtxt(file_name,unpack=True,skiprows=5)
...
if scan=='s':   
        ...
        if dim=='z':
            v1=np.array([-3,-6,-9])   
            v0=np.array([fitm*v1[0]+fitb,fitm*v1[1]+fitb,fitm*v1[2]+fitb])
            print(v0,v1)
            k=np.zeros(len(v0))
            for i in range(0,len(v0)):
